<#
 https://contoso.spo.microsoft.scloud/sites/000001/Lists/SPOMigrationStatus
 Common 2 SPO (not OneDrive)
 ...
 Script will also update users AD properties to reflect USSEC OneDrive Url and will remove users permissions from OD on Prem.
...
$listName = "SPOMigrationStatus"

# Get the SharePoint list items
$listItems = Get-PnPListItem -List $listName

# Set the Document Librarys
$sourcelist = "Documents"
$destlist = "Documents"

# Load SharePoint DLLs
Add-Type -Path "F:\IAU_Scripts\OneDrive_Migration_Scripts\Microsoft.SharePoint.Client.dll" -ErrorAction Stop
Add-Type -Path "F:\IAU_Scripts\OneDrive_Migration_Scripts\Microsoft.SharePoint.Client.Runtime.dll" -ErrorAction Stop

# Initialize an array to store users that should be processed
$itemsToProcess = @()

# Check if the user should be skipped and add to $itemsToProcess if user should be processed
foreach ($item in $listItems) {
     $site = $item["Source"]

    # Check if the user should be skipped
    if (($item["Migrate"] -eq "Migrate" -and ($item["Processing"] -eq "Processing" -or $item["Migrate"] -eq "Migrated")) -or $item["Migrate"] -eq $null -or $item["Migrate"] -eq "") {
        Write-Output "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - Skipping migration for site/library $site because the site is already being processed, already migrated, or migrate column is blank."
        continue # Skip to the next site
    }

    # If the site meets the criteria, add to the list of sites to process
    if ($item["Migrate"] -eq "Migrate") {
        $itemsToProcess += $site
    }

  # Set Processing column to "Processing" for sites with migrate column set to "migrate"
    if ($item["Migrate"] -eq "Migrate") {
        $item["Processing"] = "Processing"
        Set-PnPListItem -List $listName -Identity $item.Id -Values @{"Processing" = "Processing"}
    }

  # Set Server column to Server processing specific site migation
...
# Main migration loop
foreach ($site in $itemsToProcess) {
    
    $item = $listItems | Where-Object { $_["Source"] -eq $site }
    $targeturl = $item.FieldValues["field_1"]
    ...
    Write-Output "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - Initiating migration for site $site"
    ...
    Add-SPMTTask -SharePointSourceSiteUrl $site -TargetSiteUrl $targetUrl -MigrateAll -SharePointSourceCredential $OnPremCredential
    ...
    if ($migrationStatus.Status -eq "Finished") {
        Write-Output "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - Migration successful for site $site"
        ...
        Set-PnPListItem -List $listName -Identity $item.Id -Values @{"Migrate" = "Migrated"; "Processing" = "";"DateMigrated" = $currentDate2}
...
if (Test-Path -Path $filename -PathType Leaf){
    ...
    } elseif ($migrationStatus.Status -contains "Error" -or $migrationStatus.Status -contains "Failed" -or $migrationStatus.Status -contains "NOT") {
        $message = "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') *** Error: Migration failed for site $site."
    ...
    Start-Sleep -Seconds 120 #Pause for 120 seconds
    Stop-SPMTMigration
...
### Add remove all permissions from SP Site for all users